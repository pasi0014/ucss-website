import { defineMessages } from "@formatjs/intl";

const scope = "app.pages.About";

export default defineMessages({
  aboutUsTitle: {
    id: `${scope}.aboutUsTitle`,
    defaultMessage: "About Us",
  },
  aboutUsParagraphTitle: {
    id: `${scope}.aboutUsParagraphTitle`,
    defaultMessage:
      "What is Ukrainian Canadian Social Services - Ottawa Branch?",
  },
  aboutUsParagraph: {
    id: `${scope}.aboutUsParagraph`,
    defaultMessage:
      "Ukrainian Canadian Social Services - Ottawa Branch is a non-profit charitable organization serving the Ukrainian community in Ottawa. Guided by the needs of individuals and families of Ukrainian community, who experience language and cultural barriers. We provide a range of support services.",
  },
  aboutUsExtra: {
    id: `${scope}.aboutUsExtra`,
    defaultMessage:
      "At this challenging time for Ukraine, we are committed to the humanitarian help both displaced Ukrainian people in Canada and people in Ukraine.",
  },
  presidentName: {
    id: `${scope}.presidentName`,
    defaultMessage: "Maryna Popovych",
  },
  presidentTitle: {
    id: `${scope}.presidentTitle`,
    defaultMessage:
      "President of Ukrainian Canadian Social Services Ottawa Branch",
  },
  vpTitle: {
    id: `${scope}.vpTitle`,
    defaultMessage: "VP of Ukrainian Canadian Social Services Ottawa Branch",
  },
  vpName: {
    id: `${scope}.vpName`,
    defaultMessage: "Michael Rynzak",
  },
  artashes: {
    id: `${scope}.artashes`,
    defaultMessage: "Markarian Artashes",
  },
  lvivTitle: {
    id: `${scope}.lvivTitle`,
    defaultMessage: "Our Representatives in Lviv",
  },
  lvivGanna: {
    id: `${scope}.lvivGanna`,
    defaultMessage: "Ganna Nadtochii",
  },
  lvivVP: {
    id: `${scope}.lvivVP`,
    defaultMessage: "Deputy Commander for Personnel, Lieutenant Colonel",
  },
  coordinatorTitle: {
    id: `${scope}.coordinatorTitle`,
    defaultMessage:
      "Coordinator of Ukrainian Canadian Social Services Ottawa Branch",
  },
  coordinatorName: {
    id: `${scope}.coordinatorName`,
    defaultMessage: "Nazar Pasika",
  },
});
