...
const SideBar: React.FC<SideBarProps> = ({ children }) => {
  const [showSidebar, setShowSidenar] = useState(false);

  const toggleSidebar = () => {
    setShowSidenar(!showSidebar);
  };

  return (
    <div className="flex min-h-screen flex-row bg-gray-100 text-gray-800">
      <button
        className="lg:hidden block fixed right-0 top-0 m-4"
      ...
      <aside className="sidebar lg:w-2/12 md:w-3/12 w-4/12 -translate-x-full transform bg-white transition-transform duration-150 ease-in md:translate-x-0 md:shadow-md">
